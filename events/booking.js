...
exports.createCheckoutSession = async (event) => {
    ...
    try {
        ...
        const lineItems = bookings.flatMap(b => {
            // Each seat becomes a line item
            return b.seatNumber.map(seat => ({
                price_data: {
                    currency: "cad",
                    product_data: {
                        name: `Show ${b.shwID} - Seat ${seat}`
                    },
                    unit_amount: b.price * 100 // in cents
                },
                quantity: 1
            }));
        });
        ...
        return response(200, { sessionId: session.id });
